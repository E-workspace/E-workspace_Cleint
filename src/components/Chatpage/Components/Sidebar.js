...
export default function Sidebar() {
  ...
  const storeNotificationInLocalStorage = (notification) => {
    console.log(notification, "localnoti")
    try {
      ...
      try {
        ...
        const formattedConversations = response.data.map((chat) => {
          ...
          if (!isGroupChat) {
            ...
            chatName = otherUser ? otherUser.username || otherUser.name : "";
          }
          ...
          // Get the profile image of the other user in one-on-one chats or in group chats
          const Image = chat.users
            .filter(
              (obj) =>
                obj._id !== localStorage.getItem("userId") && obj.profileImage
            )
            .map((obj) => obj.profileImage)[0];
  ...
  const filteredConversations = conversations.filter((conversation) => {
    ...
    return conversation.name && conversation.name.toLowerCase().includes(searchTerm.toLowerCase());
  });
    ...
    <div className="sidebar-container">
      <div className={"sb-header" + (lightTheme ? "" : " dark")}>
        <div className="other-icons">
          ...
          >
          {console.log(notifications, "noytiii")}
            {notifications.length > 0 ? (
              ...
              notifications.map((notification, index) => (
                ...
                <MenuItem
                  key={index}
                  onClick={() => {
                     console.log(notification, "notificationsssssssssss");
                    handleClose();
                ...
                {notification.isGroupChat 
                  ? (notification.ChatName ? notification.ChatName[0] : 'G') 
                  : (notification.sender ? notification.sender[0] : 'U')}
              </Avatar>
              {notification.isGroupChat 
                ? notification.ChatName || 'Group Chat'
