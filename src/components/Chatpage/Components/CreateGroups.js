...
export default function CreateGroups() {
    ...
    const fetchUsers = async () => {
      try {
        ...
        const usersWithoutPasswords = response.data.map(
          ...
          console.log(usersWithoutPasswords, "without passwords")
  ...
  const filteredUsers = users.filter((user) => {
    console.log(user, 'users in filter');
  
    return (
      (user.username && user.username.trim() !== '' && user.username.toLowerCase().includes(searchTerm.toLowerCase())) ||
          ...
          filteredUsers.map((user) => (
            ...
            >
              ...
              {user.image ? (
                ...
                <p className="con-icon">{user.username ? user.username[0] : 'U'}</p>
              ...
              <div>
                ...
                <Checkbox
                  onChange={(e) =>
                    handleCheckboxChange(user._id, user.username, e.target.checked)
                  }
