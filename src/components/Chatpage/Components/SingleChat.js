  ...
  useEffect(() => {
    const handleNewMessage = (newMessage) => {
      ...
      if (ChatInfo._id === newMessage.chat._id) {
        ...
        Ring();
        toast(`${newMessage.sender.username}: ${newMessage.content}`, {
    ...
    if (ChatInfo._id) {
      ...
      .then(({ data }) => {
        if (data.length === 0) {
          ...
          if (data[0]?.chat?.users && !data[0].chat.users.includes(userId)) {
            navigate("/dashboard");
          }
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.log(data, "from singlechat member props")
        setGroupData(data);
        setGroupLoading(false);
      ...
      >
        <div>
          {groupLoading ? (
            ...
            groupData && (
              <div>
                {groupData.userNames.map((userMapping, index) => (
                  <div key={index}>
                    <Members
                      name={userMapping}
                      isAdmin={
