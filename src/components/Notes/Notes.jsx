...
useEffect(() => {
  ...
  const fetchNotes = async (email, username) => {
    ...
    try {
      setLoading(true);
      const response = await axios.post(`https://eworkspacems2-loszcsdz.b4a.run/api/Getnotes`, {
        email,
        username,
      }, {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (originalId) {
        ...
        await axios.put(`https://eworkspacems2-loszcsdz.b4a.run/api/editNote/${originalId}`, payload, {
          headers: {
        ...
        // Add new note
        await axios.post(`https://eworkspacems2-loszcsdz.b4a.run/api/Savenotes`, payload, {
          headers: {
            "Content-Type": "application/json",
            "X-CSRF-Token": csrfToken,
          },
  ...
  const handleDelete = async (noteId) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        await axios.delete(`https://eworkspacems2-loszcsdz.b4a.run/api/deleteNote/${noteId}`, {
          headers: {
            "X-CSRF-Token": csrfToken,
          },
          withCredentials: true,
